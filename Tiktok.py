...
class Zefoy:
        ...
        while True:
            ...
            if 'Session expired. Please re-login' in self.video_info:
                ...
                if zefoy==0:self.find_video()
                    ...
                    print('\033[31mIP ĐÃ BỊ CHẶN')
                _=time.time()
                while time.time()-2<_+zefoy:
                    t-=1
                    print('\033[31m[VINH]=>Vui lòng chờ \033[1;32m{0}\033[31m '.format(t)+'giây\033[1;32m',end='\r')
                    time.sleep(1)
                continue
    ...
    def use_service(self):
        ...
        if 'Session expired. Please re-login' in res:
            print('\033[31mPhiên hết hạn. Đang đăng nhập lại')
            self.send_captcha()
            return ""
        elif 'Too many requests. Please slow' in res:
            time.sleep(3)
        elif 'service is currently not working' in res:
            return ('\033[31mDịch vụ hiện không hoạt động, hãy thử lại sau')
        else:
    ...
    def get_video_id(self, url_ = None, set_url=True):
        ...
        if request.text == '':
            print('\033[31mLink video không hợp lệ')
            return False
        else:
            json_ = request.json()
        if 'author' not in json_:
            print(f'\033[31m{self.url} | Link video không hợp lệ')
            return False
        if set_url:
    ...
    def select_service(self):
        while True:
            ...
            tim = "\033[1;35m"
            self.get_table()
            print(f"{xanh_la}Nhập Số\033[1;37m:", end=' ')
            service_id = input()
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                print(f'\033[1;31mLỖI NGHIÊM TÚC | THỬ LẠI SAU | {e}')
                time.sleep(10)
